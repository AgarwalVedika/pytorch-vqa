...
def main(args):
    ...
    if args.model_type == 'no_attn':
        ...
        # model_types = ['finetuning_CNN_LSTM_ratio_mix_orig:edit_0.3:0.7', 'finetuning_CNN_LSTM_ratio_mix_orig:edit_0.7:0.3', \
        ...
        model_types = ['finetuning_CNN_LSTM_ratio_mix_no_repeat_edit_ids_orig:edit_0.3:0.7']  #'finetuning_CNN_LSTM_ratio_mix_no_repeat_edit_ids_orig:edit_0.3:0.7'

        epoch_list2 = [0,17,4,2,0,14,8]
        epoch_list = [41, 17, 27, 2, 16, 17, 17]
        epoch_list = [17]
        ...
        for idx,model_type in enumerate(model_types):
            ...
            #model_path = os.path.join(model_path_folder, 'epoch_49.pth')
            model_path = os.path.join(model_path_folder, 'epoch_{}.pth'.format(epoch_list[idx]))
            #res_json_folder = os.path.join('/BS/vedika3/nobackup/pytorch-vqa/finetuning_logs_diff_str_using_epoch49/' + model_type + '/' + config.ques_type + '/' + config.test_data_split )
            res_json_folder = os.path.join('/BS/vedika3/nobackup/pytorch-vqa/finetuning_logs_diff_str_using_epoch' + str(epoch_list[idx]) + '/' + model_type + '/' + config.ques_type + '/' + config.test_data_split)
